            ...
            setTimeout(() => {
              (async () => {
                ...
                if (messages.length > 0) {
                  ...
                  await checkResposta(client, clientePath, chatId, answer);
...
async function identifyNameAndInterest(client: wppconnect.Whatsapp, chatId: string) {
  ...
  const namePrompt = env.find(linha => linha.startsWith('NAME_PROMPT='))?.split('=')[1] || ``;
  const interestPrompt = env.find(linha => linha.startsWith('INTEREST_PROMPT='))?.split('=')[1] || ``;
  const orçamentoPrompt = env.find(linha => linha.startsWith('ORCAMENTO_PROMPT='))?.split('=')[1] || ``;
...
async function generateSummary(client: wppconnect.Whatsapp, chatId: string) {
  const filePath = path.join(process.cwd(), `clientes`, cliente, `Chats`, `Historico`, `${chatId}`, `${chatId}.json`);
  const conversation = fs.readFileSync(filePath, `utf-8`);
  const summaryPrompt = env.find(linha => linha.startsWith('SUMMARY_PROMPT='))?.split('=')[1] || ``;
