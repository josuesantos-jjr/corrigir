...
import { sendMessageZap, updateLeadData } from '../multi'; 
import { saveMessageToFile } from '../multi';
...
const extrairMensagem = (listaNome: string, nome: string): string => {
  const filePath = path.join(process.cwd(), 'src', 'disparo', 'abordagens', `${listaNome}.json`);
  try {
...
const filtrarNumerosExistentes = async (listaNome: string, numerosExtraidos: string[], nome: string) => {
  const arquivoDisparo = path.join(process.cwd(), 'src', 'disparo', 'listas', 'disparar', `${listaNome}.json`);
...
const LCFdados = async (listaNome: string, numerosExtraidos: string[], nome: string) => {
  const arquivoDisparo = path.join(process.cwd(), 'src', 'disparo', 'listas', 'disparar', `${listaNome}.json`);
  const arquivoDownload = path.join(process.cwd(), 'src', 'disparo', 'listas', 'download', `${listaNome}.json`);
  const nomecontato = getContactName;
...
const atualizarListaComDisparo = async (listaNome: string, numeroExtraido: string, dataDisparo: string, sucesso: boolean) => {
  const arquivoDisparo = path.join(process.cwd(), 'src', 'disparo', 'listas', 'disparar', `${listaNome}.json`);
  ...
  try {
    ...
    if (indice !== -1) {
      ...
      const nome = getContactName(numeroExtraido, listaNome);
...
const getContactName = (telefone: string, listaNome: string): string => {
  const filePath = path.join(process.cwd(), 'src', 'disparo', 'listas', 'download', `${listaNome}.json`);
  try {
    const data = fs.readFileSync(filePath, 'utf8');
    const jsonData = JSON.parse(data);
...
const salvarLog = (mensagem: string) => {
  const logPath = path.join(process.cwd(), 'src', 'disparo', 'erros', 'log.txt');
...
const registrarEstado = (estado: any) => {
...
const salvarEstado = (estado: any) => {
  const estadoPath = path.join(process.cwd(), 'src', 'disparo', 'estado.json');
  fs.writeFileSync(estadoPath, JSON.stringify(estado, null, 2));
};

// Função para carregar o estado salvo
const carregarEstado = (): any => {
  const estadoPath = path.join(process.cwd(), 'src', 'disparo', 'estado.json');
...
let disparoHoje = 0;
const arquivosDisparo = fs.readdirSync(path.join(process.cwd(), 'src', 'disparo', 'listas', 'disparar'));
const hoje = new Date();
...
for (const arquivo of arquivosDisparo) {
  ...
  if (path.extname(arquivo) === '.json') {
    const filePath = path.join(process.cwd(), 'src', 'disparo', 'listas', 'disparar', arquivo);
    try {
      const data = fs.readFileSync(filePath, 'utf8');
...
const numeroJaDisparado = async (listaNome: string, telefone: string): Promise<boolean> => {
  const arquivoDisparo = path.join(process.cwd(), 'src', 'disparo', 'listas', 'disparar', `${listaNome}.json`);
...
const dispararMensagens = async (clients: any[]) => {
  let estado = carregarEstado() || {
    ultimoDiaDisparo: '',
    diasRestantesAquecimento: 0,
    quantidadeMensagensDia: 0,
    diaspassados: 0,
    listaAtual: '',
    indiceTelefoneAtual: ''
  };
  ...
  try {
    ...
    const regrasDisparoPath = path.join(process.cwd(), 'src', 'disparo', '.regrasdisparo');
    ...
    const arquivoListas = path.join(process.cwd(), 'src', 'disparo', '.listas');
    const listas = await extrairListas(arquivoListas);
    const diasDaSemana = ['domingo', 'segunda', 'terça', 'quarta', 'quinta', 'sexta', 'sábado'];
    ...
    while (true) {
      ...
      const numerosFiltrados = await LCFdados(lista.nome, numerosExtraidos, lista.nome);
      
      await filtrarNumerosExistentes(lista.nome, numerosExtraidos, lista.nome);
      ...
      for (let i = estado.indiceTelefoneAtual; i < numerosFiltrados.length; i++) {
        ...
        const jaFoiDisparado = await numeroJaDisparado(lista.nome, telefone);
        ...
        if (!dentroDoHorario(horarioInicial, horarioFinal)) {
          ...
          console.log(`Fora do horário permitido. Aguardando até ${horarioInicial}...`);
          salvarLog(`Fora do horário permitido. Aguardando até ${horarioInicial}`);
          console.log(`Dias restantes de aquecimento: ${diasRestantesAquecimento}`);
          salvarLog(`Dias restantes de aquecimento: ${diasRestantesAquecimento}`);
        ...
        const clientToUse = clients[i % clients.length]; 

        const temWhatsApp = await verificarContaWhatsApp(clientToUse, telefone); // Verifica se o número tem WhatsApp
        ...
        if (contadorMensagens >= quantidadeMensagensDia) {      
          ...
          if (disparoHoje >= quantidadeMensagensDia) {
            ...
            diasRestantesAquecimento --;
            console.log(`Dias restantes de aquecimento 1: ${diasRestantesAquecimento}`);
            salvarLog(`Dias restantes de aquecimento 1: ${diasRestantesAquecimento}`);
        ...
        try {
          // Get the contact name from the download folder
          const nome = getContactName(telefone, lista.nome);

          const mensagem = extrairMensagem(lista.nome, nome); // Pass the contact name to the extrairMensagem function

          // **Intercalando os clientes**
          const clientToUse = clients[i % clients.length]; // Use modulo operator to cycle through clients

          await sendMessageZap(clientToUse, chatId, mensagem); // Verifica se a mensagem foi enviada com sucesso

          updateLeadData(chatId, 'clientUsed', `${clientToUse}`);
          ...
          const dataDisparo = new Date().toLocaleDateString('pt-BR', { hour: '2-digit', minute: '2-digit' }); // Formata a data e hora
          await atualizarListaComDisparo(lista.nome, telefone, dataDisparo, !!sucessoDisparo); // Atualiza a lista com o status do disparo
          salvarEstado(estado);
          salvarLog(`Mensagem enviada para ${telefone} | ${contadorMensagens} de ${quantidadeMensagensDia} | ${clientToUse}`);
        } catch (error) {
          ...
          await atualizarListaComDisparo(lista.nome, telefone, "Não", false);
        }
        //Salvar historico chatid
        const nome = getContactName(telefone, lista.nome);
        const mesage = extrairMensagem(lista.nome, nome);
        await saveMessageToFile(clientToUse, chatId, mesage, 'IA');  
        ...
        if (contadorMensagens % quantidadeSequencia === 0) {
            registrarEstado(estado); // Log the state before the hourly pause
      ...
      registrarEstado(estado); // Log the state before the daily pause
    ...
    await iniciarDisparoComRecuperacao(clients); 
    salvarLog(`Disparo - Erro ao disparar mensagens: ${error}`); 
  };
};

// Função para iniciar o processo de disparo com recuperação de erros
const iniciarDisparoComRecuperacao = async (client: any) => {
  while (true) {
    try {
