...
const getLastMessageDate = (chatId: string, clientePath: string, listaNome: string): Date | undefined => {
  const filePath = path.join(clientePath, 'listas', `${listaNome}.json`);
  try {
...
const getPrompt = (nivel: string, clientePath: string): string => {
  ...
  try {
    ...
    const prompts = data.split('\n').reduce((acc, line) => {
      ...
      if (key && value && key.trim() === nivel) { // Verifica se a chave corresponde ao nível desejado
        acc = value.trim().replace(/`/g, ''); // Atribui o valor diretamente a acc
      }
      return acc;
    }, ''); // Define a string vazia como valor inicial de acc
...
const processNivel2 = async (client: any, clientePath: string): Promise<void> => {
  console.log('Iniciando processamento do nível 2...');
  ...
  for (const arquivo of listaArquivos) {
    ...
    if (fs.statSync(listaPath).isFile() && path.extname(listaPath) === '.json') {
      ...
      const numerosFiltrados = listaData.filter((item: any) => {
        ...
        return item['Nivel FollowUp'] === '2' && (!dados || dados.interest !== 'Não');
      });
      ...
      for (const item of numerosFiltrados) {
        ...
        if (lastMessageDate && !isWithinFifteenDays(lastMessageDate)) {
          ...
          if (isWithinAllowedTime(clientePath)) {
            ...
            console.log('Informações do cliente:', informações);
  ...
  console.log('Processamento do nível 2 finalizado.');
};

export { processNivel2 };
